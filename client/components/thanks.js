
export const thanks = [
  "Mitch & Ali from Maggle Creative",
  "Oscar Keys from the NZVRARA Society",
  "Steven Knightly from InGame",
  "Hori Te Ariki Mataki from Ariki Creative",
  "Vincent Egan from Maui Studios",
  "Dougal Stott from Enspiral Dev Academy",
  "Ross Jourdain, Harrison Symes, Kelly Keating, Rohan Wakefield and Sarrah Grace from Enspiral Dev Academy",
  "Whitiaua Black",
  "Alex Mitcalfe Wilson",
  "The members of the Kōkako 2018 cohort at Enspiral Dev Academy",
  "Louis the dog",
  "Archives New Zealand",
  "Island Bay",
  "Mount Victoria",
  "Black Dog Brewery",
  "Feijoas, Mandarins, and Berocca for all the cold-delaying vitamins",
  "Legend of Tāwhirimātea (c) Wiremu Grace, sourced from Ministry of Education"  
]
